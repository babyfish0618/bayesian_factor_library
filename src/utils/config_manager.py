...
class EvaluationConfig:
    ...
    marginal_contribution: Dict[str, Any] = field(default_factory=lambda: {
        ...
        'scoring': {
            'weights': {'improvement': 0.4, 'correlation': 0.3, 'quality': 0.2, 'feasibility': 0.1},
            'grade_scores': {'high': 1.0, 'medium': 0.7, 'low': 0.4, 'fallback': 0.1},
            'quality_thresholds': {'high': 2.0, 'medium': 1.0, 'low': 0.5},
            'feasibility_scores': {'replace': 1.0, 'no_replace': 0.3},
            'correlation_low_ratio': 0.5,
            'decision_success_score': 0.7,
    ...
    def validate(self) -> bool:
        ...
        try:
            ...
            # 验证阈值合理性（按存在字段校验，兼容精简配置）
            selected = self.success_thresholds.get('selected', {})
            unselected = self.success_thresholds.get('unselected', {})

            s_icir = selected.get('icir')
            u_icir = unselected.get('icir')
            if s_icir is not None and u_icir is not None and s_icir >= u_icir:
                raise ValueError("没选中因子ICIR阈值必须大于选中因子阈值")
            
            s_ls = selected.get('ls_return_annual')
            u_ls = unselected.get('ls_return_annual')
            if s_ls is not None and u_ls is not None and s_ls >= u_ls:
...
class ConfigManager:
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_thresholds': {
                ...
                'unselected_icir': self.config.success_thresholds.get('unselected', {}).get('icir'),
                'selected_ls_return': self.config.success_thresholds.get('selected', {}).get('ls_return_annual'),
                'unselected_ls_return': self.config.success_thresholds.get('unselected', {}).get('ls_return_annual')
            }
        }
    ...
    def print_summary(self):
        ...
        if thresholds.get('unselected_icir') is not None:
            print(f"  没选中ICIR: >{thresholds.get('unselected_icir')}")
        s_ls = thresholds.get('selected_ls_return')
        u_ls = thresholds.get('unselected_ls_return')
        if s_ls is not None:
            print(f"  选中年化收益: >{s_ls:.1%}")
        if u_ls is not None:
            print(f"  没选中年化收益: >{u_ls:.1%}")
